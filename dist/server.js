...
export async function extractTextFromPptxBuffer(pptxFile) {
    try {
        ...
        slideEntries.sort((a, b) => {
            const numA = Number.parseInt(a.entryName.match(/slide([0-9]+)\.xml/)?.[1] ?? "0");
            const numB = Number.parseInt(b.entryName.match(/slide([0-9]+)\.xml/)?.[1] ?? "0");
            return numA - numB;
        });
        ...
        for (const entry of slideEntries) {
            ...
            const slideNumber = entry.entryName.match(/slide([0-9]+)\.xml/)?.[1] ?? -1;
            allText += `\n===== 슬라이드 ${slideNumber} =====\n`;
...
function extractTextFromSlide(slideObj) {
    let text = "";
    try {
...
export async function extractTextFromS3Object(s3Body) {
    try {
        const chunks = [];
